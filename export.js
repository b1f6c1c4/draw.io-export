  ...
  if (req.body.html) {
    ...
    try {
      html = decodeURIComponent(
        zlib.inflateRawSync(
          new Buffer(decodeURIComponent(html), 'base64'),
        ).toString(),
      );
      ...
      if (browser != null) {
        if (errorOccurred) { browsersPool.destroy(browser); } else { browsersPool.release(browser); }
      }
    }
  } else {
    let xml;
      ...
      || xml.indexOf('<!--[if IE]><meta http-equiv') == 0)) // TODO not tested!
    {
      try {
        ...
        let divs = doc.documentElement
          .getElementsByTagName('div');

        if (divs != null && divs.length > 0
          && (divs.item(0).attributes
            .getNamedItem('class').nodeValue) == 'mxgraph') {
